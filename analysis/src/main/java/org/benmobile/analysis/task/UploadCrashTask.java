...
public class UploadCrashTask extends BaseTask {

	private static final String TAG = "UploadCrashTask";

	private ArrayList<String> crashIds = new ArrayList<String>();
	private HashMap<String, ArrayList<String>> ids = new HashMap<String, ArrayList<String>>();

	public UploadCrashTask(Handler callback) {
		super(callback);
		// TODO Auto-generated constructor stub
		ids.put(MBLogCrash.class.getName(), crashIds);
	}
	...
	public void doTask() {
		...
		Logger.info("UploadCrashTask", "data: "+data);
//		try {
...
//		};
	...
	private String loadLogFromDatabase() {
		if (SyknetMobileLog.mainDatabaseHelper != null) {
			List<MBLogCrash> logCrash = SyknetMobileLog.mainDatabaseHelper
					.readAll(LogDatabaseHelper.TABLE_MB_LOG_CRASH,
							MBLogCrash.class);
			JSONObject log = new JSONObject();
			try {
				if (ValueUtils.isListEmpty(logCrash))
				{return "";}
				log.put(LogDatabaseHelper.TABLE_MB_LOG_CRASH, logToString(logCrash));
				return log.toString();
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				Logger.exception(e);
	...
	private void deleteLogs(){
		if (SyknetMobileLog.mainDatabaseHelper != null){
			SyknetMobileLog.mainDatabaseHelper.delete(LogDatabaseHelper.TABLE_MB_LOG_CRASH, crashIds);
		}
	}
