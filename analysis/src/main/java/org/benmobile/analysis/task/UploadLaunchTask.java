...
public class UploadLaunchTask extends BaseTask {

	private static final String TAG = "UploadLogTask";
	private ArrayList<String> launchIds = new ArrayList<String>();
	private HashMap<String, ArrayList<String>> ids = new HashMap<String, ArrayList<String>>();

	public UploadLaunchTask(Handler callback) {
		super(callback);
		// TODO Auto-generated constructor stub
		ids.put(MBLogLaunch.class.getName(), launchIds);
	}
	...
	public void doTask() {
		...
		if (SyknetMobileLog.DEBUG){
			Log.e("UploadLaunchTask", "data: "+data);
		}
//		try {
...
//		};
	...
	private String loadLogFromDatabase() {
		if (SyknetMobileLog.mainDatabaseHelper != null) {
			List<MBLogLaunch> logLaunch = SyknetMobileLog.mainDatabaseHelper
					.readAll(LogDatabaseHelper.TABLE_MB_LOG_LAUNCH,
							MBLogLaunch.class);
			JSONObject log = new JSONObject();
			try {
				if (ValueUtils.isListEmpty(logLaunch))
				{return "";}
				log.put(LogDatabaseHelper.TABLE_MB_LOG_LAUNCH, logToString(logLaunch));
				return log.toString();
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				return "";
	...
	private void deleteLogs(){
		if (SyknetMobileLog.mainDatabaseHelper != null){
			SyknetMobileLog.mainDatabaseHelper.delete(LogDatabaseHelper.TABLE_MB_LOG_LAUNCH, launchIds);
		}
	}
