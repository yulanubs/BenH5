...
//		db.execSQL(CREATE_MB_LOG_DEVICE);
	...
	public <T extends BaseDatabaseObj>List<T> readAll(String tableName,Class<T> clazz){
		synchronized (lock) {
			...
			c.moveToFirst();
			Logger.debug("LogDatabaseHelper", "readAll: "+tableName);
			Logger.debug("LogDatabaseHelper", "readAll: "+c.getCount());
			while (!c.isAfterLast()) {
				Logger.warn("readAll", "loop1: "+c.getPosition());
				try {
					T o = clazz.getConstructor().newInstance();
					Logger.info("LogDatabaseHelper", "readAll: "+o.getDatabaseTableName());
					...
					for(int i=0;i<fs.length;i++){
						...
						Logger.info("DatabaseKey", key);
						String value = c.getString(c.getColumnIndex(key));
						Logger.info("LogDatabaseHelper", "field value: "+value);
						f.set(o, value);
					}
					rets.add(o);
			...
			c.close();
			Logger.info("LogDatabaseHelper", "rets length: "+rets.size());
			return rets;
		}
	}
}
